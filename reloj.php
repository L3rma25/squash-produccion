...
<h1 class="mt-4">Cancha Norte</h1>
<form name="form_reloj" action='reloj2.php' method='POST' id='frmdo' enctype='multipart/form-data'>
...
include ('conexion.php');
   ...
   $sqlemp="SELECT * FROM detalle_canchas where activo='Si' and hora_inicio!='' and id_cancha='1'";
   $resemp = mysqli_query($conn,$sqlemp) or die(mysqli_error());
   $filasemp = mysqli_num_rows($resemp);
   
   if($filasemp==1){
	   
          echo"<div class='card-body'><div class='card bg-danger text-white mb-4'>CANCHA OCUPADA</div></div>";
          echo'<input type="submit" class="btn btn-primary" name="Terminar" value="Terminar">';
            
   }else{
echo'<br><input type="text" class="form-control" name="cliente1" placeholder="Ingresa Nombre del Cliente">';
         echo"<div class='card-body'><div class='card bg-success text-white mb-4'>CANCHA DESOCUPADA</div></div>";
		 
		 echo'<input type="submit" class="btn btn-primary" name="Iniciar" value="Iniciar">';
   }
?>

<h1 class="mt-4">Cancha Sur</h1>
	<?php
   
include ('conexion.php');

   $sqlemp="SELECT * FROM detalle_canchas where activo='Si' and hora_inicio!='' and id_cancha='2'";
   $resemp = mysqli_query($conn,$sqlemp) or die(mysqli_error());
   $filasemp = mysqli_num_rows($resemp);
   if($filasemp==1){
   ...
   echo'<br><input type="text" class="form-control" name="cliente2" placeholder="Ingresa Nombre del Cliente">';
         echo"<div class='card-body'><div class='card bg-success text-white mb-4'>CANCHA DESOCUPADA</div></div>";
